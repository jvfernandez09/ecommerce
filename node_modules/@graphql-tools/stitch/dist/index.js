...
import { collectSubFields, memoize1, memoize2, memoize3, parseSelectionSet, mapMaybePromise, isSome, collectFields, getImplementingTypes, filterSchema, getRootTypeNames, mergeDeep, getDescription, createNamedStub, createStub, getRootTypeMap, getRootTypes, inspect, rewireTypes, getOperationASTFromRequest, getDefinedRootType } from '@graphql-tools/utils';
import { isAbstractType, Kind, getNamedType, GraphQLList, isObjectType, isInterfaceType, isLeafType, isInputObjectType, isUnionType, print, isScalarType, isCompositeType, isNonNullType, isEnumType, isListType, getNullableType, isNullableType, GraphQLObjectType, GraphQLInputObjectType, GraphQLInterfaceType, GraphQLUnionType, GraphQLEnumType, GraphQLScalarType, GraphQLDirective, valueFromASTUntyped, getDirectiveValues, GraphQLDeprecatedDirective, DirectiveLocation, isNamedType, isIntrospectionType, isDirective, isSpecifiedScalarType, specifiedDirectives, GraphQLSchema, extendSchema } from 'graphql';
...
function filterIsolatedSubschema(subschemaConfig, computedFieldTypes = {}) {
  const queryRootFields = {};
  function listReachableTypesToIsolate(subschemaConfig2, type, typeNames = /* @__PURE__ */ new Set()) {
    if (isScalarType(type)) {
      return typeNames;
    } else if ((isObjectType(type) || isInterfaceType(type)) && subschemaConfig2.merge?.[type.name]?.selectionSet) {
  ...
  const filteredSchema = filterSchema({
    ...
    objectFieldFilter: (typeName, fieldName, config) => {
      ...
      return subschemaConfig.merge[typeName] == null || subschemaConfig.merge[typeName]?.fields?.[fieldName] != null || (subschemaConfig.merge[typeName]?.keyFieldNames ?? []).includes(
        fieldName
      );
    },
    interfaceFieldFilter: (typeName, fieldName, config) => {
      ...
      const isIsolatedFieldName = typesForInterface[typeName].some(
        (implementingTypeName) => isIsolatedField(
      ...
      return isIsolatedFieldName || isComputedFieldType || typesForInterface[typeName].some(
        (implementingTypeName) => (subschemaConfig.merge[implementingTypeName]?.keyFieldNames ?? []).includes(fieldName)
